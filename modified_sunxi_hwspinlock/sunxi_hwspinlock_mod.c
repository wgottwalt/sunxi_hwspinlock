...
{
	struct sunxi_hwspinlock_mod_data *priv = seqf->private;
...
static void sunxi_hwspinlock_mod_debugfs_init(struct sunxi_hwspinlock_mod_data *priv)
{
...
static int sunxi_hwspinlock_mod_trylock(struct hwspinlock *lock)
{
...
static void sunxi_hwspinlock_mod_unlock(struct hwspinlock *lock)
{
	void __iomem *lock_addr = lock->priv;
...
static const struct hwspinlock_ops sunxi_hwspinlock_mod_ops = {
	.trylock	= sunxi_hwspinlock_mod_trylock,
	.unlock		= sunxi_hwspinlock_mod_unlock,
};

static int sunxi_hwspinlock_mod_probe(struct platform_device *pdev)
{
	struct sunxi_hwspinlock_mod_data *priv;
	...
	err = hwspin_lock_register(priv->bank, &pdev->dev, &sunxi_hwspinlock_mod_ops,
	...
	sunxi_hwspinlock_mod_debugfs_init(priv);
...
static int sunxi_hwspinlock_mod_remove(struct platform_device *pdev)
{
	struct sunxi_hwspinlock_mod_data *priv = platform_get_drvdata(pdev);
	int err;
...
static const struct of_device_id sunxi_hwspinlock_mod_ids[] = {
	{ .compatible = "allwinner,sun8i-hwspinlock-mod", },
	{ .compatible = "allwinner,sun50i-hwspinlock-mod", },
	{},
};
MODULE_DEVICE_TABLE(of, sunxi_hwspinlock_mod_ids);

static struct platform_driver sunxi_hwspinlock_mod_driver = {
	.probe	= sunxi_hwspinlock_mod_probe,
	.remove	= sunxi_hwspinlock_mod_remove,
	.driver	= {
		.name		= DRIVER_NAME,
		.of_match_table	= sunxi_hwspinlock_mod_ids,
	},
};

static int __init sunxi_hwspinlock_mod_init(void)
{
	return platform_driver_register(&sunxi_hwspinlock_mod_driver);
}
postcore_initcall(sunxi_hwspinlock_mod_init);

static void __exit sunxi_hwspinlock_mod_exit(void)
{
	platform_driver_unregister(&sunxi_hwspinlock_mod_driver);
}
module_exit(sunxi_hwspinlock_mod_exit);
